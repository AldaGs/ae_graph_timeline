...
| M4.6 | Pre-M7 editing completion | **IN PROGRESS** — AE Project-panel footage drag/drop is implemented offline using existing item IDs; installed AE acceptance, unmanaged-layer Import/Ignore, Monaco expression editing, installed-effect catalogue, and keyboard connection acceptance remain open. See `docs/M4.6.md`. |
