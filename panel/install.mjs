// Installs the panel into the Adobe CEP extensions folder so After Effects
// loads it as an unsigned dev panel. Adapted from cep-spike/install.mjs, which
// was adapted from ExtendBlueNode's scripts/install-cep.mjs.
//
// Unlike the spike, this panel is BUILT: run `npm run build` first, or the
// manifest's dist/index.html will not be there and After Effects will show an
...
const BUNDLE_ID = 'com.nodetimeline.panel';
...
// A junction to a panel that was never built is the same silent blank panel as
...
console.log('\n  Next: restart After Effects, then Window > Extensions > Node Timeline');
console.log('  DevTools while the panel is open: http://localhost:8091\n');
